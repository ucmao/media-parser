...
### 4. 运行交互式真实样本回归验证工具
[tests/manual_verify_parsers.py](file:///Users/leo/Projects/media-parser/tests/manual_verify_parsers.py) 支持对 36 个平台的多种形态（短链、长链、视频、图集等）发起真实网络请求与字段契约校验：

```bash
# 1. 快速冒烟测试 (每个平台只测 1 条最具代表性的链接，极速完成 36 平台基准健康检查)
