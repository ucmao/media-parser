...
class DBManager:
    ...
    def filter_new_user_actions(self, user_id: int, video_ids: list, action_type: str) -> list:
        """
        记录用户对视频的行为，并返回本次“首次行为”的视频ID列表。
        同一用户对同一视频同一行为只会插入一次（依赖 user_video_actions 上的唯一索引）。
        """
        if not self.conn:
            raise Exception("Database not connected")
        if not video_ids or not isinstance(video_ids, list):
        ...
        try:
            insert_sql = """
                INSERT IGNORE INTO user_video_actions (user_id, video_id, action_type)
                VALUES (%s, %s, %s)
            """
            for vid in video_ids:
                cursor.execute(insert_sql, (user_id, vid, action_type))
                # rowcount == 1 说明是首次行为，被成功插入
            ...
            self.conn.rollback()
            logger.error(f"记录用户行为失败：{str(e)}", exc_info=True)
            raise e
        finally:
            cursor.close()
