...
class DataStorageManager:
    ...
    @staticmethod
    def filter_new_user_actions(user_id: int, video_ids: list, action_type: str) -> list:
        """
        通过 user_video_actions 表按用户+视频+行为去重，返回本次应当计分的视频ID列表。
        """
        manager = DataStorageManager._connect_db()
        try:
            return manager.filter_new_user_actions(user_id=user_id, video_ids=video_ids, action_type=action_type)
        finally:
            DataStorageManager._disconnect_db(manager)
